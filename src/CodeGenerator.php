...
use tlopasha\cotACl\Models\ModuleFieldTypes;
use tlopasha\cotACl\Helpers\LvHelper;
use tlopasha\cotACl\Models\Menu;
...
class CodeGenerator
{
	...
	**/
	public static function createController($config, $comm = null) {
        ...
        LvHelper::log("info", "Creating controller...", $comm);
        $md = file_get_contents($templateDirectory."/controller.template");
        ...
        file_put_contents(base_path('app/Http/Controllers/LaravelVueAdmin/'.$config->controllerName.".php"), $md);
    }
    ...
    public static function createModel($config, $comm = null) {
        ...
        LvHelper::log("info", "Creating model...", $comm);
        $md = file_get_contents($templateDirectory."/model.template");
    ...
    public static function createViews($config, $comm = null) {
        ...
        LvHelper::log("info", "Creating views...", $comm);
        // Create Folder
        @mkdir(base_path("resources/views/LaravelVueAdmin/".$config->dbTableName), 0777, true);

        // ============================ Listing / Index ============================
        $md = file_get_contents($templateDirectory."/views/index.blade.template");
        ...
        $inputFields = "";
        foreach ($config->module->fields as $field) {
            $inputFields .= "\t\t\t\t\t@lv_input($"."module, '".$field['colname']."')\n";
        }
        $inputFields = trim($inputFields);
        $md = str_replace("__input_fields__", $inputFields, $md);

        file_put_contents(base_path('resources/views/LaravelVueAdmin/'.$config->dbTableName.'/index.blade.php'), $md);

        // ============================ Edit ============================
        $md = file_get_contents($templateDirectory."/views/edit.blade.template");
        ...
        file_put_contents(base_path('resources/views/LaravelVueAdmin/'.$config->dbTableName.'/edit.blade.php'), $md);

        // ============================ Show ============================
        $md = file_get_contents($templateDirectory."/views/show.blade.template");
        ...
        $displayFields = "";
        foreach ($config->module->fields as $field) {
            $displayFields .= "\t\t\t\t\t\t@lv_display($"."module, '".$field['colname']."')\n";
        }
        $displayFields = trim($displayFields);
        $md = str_replace("__display_fields__", $displayFields, $md);

        file_put_contents(base_path('resources/views/LaravelVueAdmin/'.$config->dbTableName.'/show.blade.php'), $md);
    }
    ...
    public static function appendRoutes($config, $comm = null) {
        ...
        LvHelper::log("info", "Appending routes...", $comm);
        ...
        $md = file_get_contents($templateDirectory."/routes.template");
    ...
    public static function addMenu($config, $comm = null) {
        ...
        LvHelper::log("info", "Appending Menu...", $comm);
        ...
        // $menu = '<li><a href="{{ url(config("laravelVueAdmin.adminRoute") . '."'".'/'.$config->dbTableName."'".') }}"><i class="fa fa-cube"></i> <span>'.$config->moduleName.'</span></a></li>'."\n".'            <!-- LvMenus -->';
        // $md = file_get_contents(base_path('resources/views/LaravelVueAdmin/layouts/partials/sidebar.blade.php'));
        // $md = str_replace("<!-- LvMenus -->", $menu, $md);
        // file_put_contents(base_path('resources/views/LaravelVueAdmin/layouts/partials/sidebar.blade.php'), $md);
    }
	...
	**/
	public static function generateMigration($table, $generate = false, $comm = null)
	{
		...
		LvHelper::log("info", "Model:\t   ".$modelName, $comm);
		LvHelper::log("info", "Module:\t   ".$moduleName, $comm);
		LvHelper::log("info", "Table:\t   ".$dbTableName, $comm);
		LvHelper::log("info", "Migration: ".$migrationName."\n", $comm);
        ...
        if($generate) {
            ...
            if(isset($module)) {
				LvHelper::log("info", "Module exists :\t   ".$moduleName, $comm);
                ...
                if($fileExists) {
					LvHelper::log("info", "Replacing old migration file: ".$fileExistName, $comm);
                    $migrationFileName = $fileExistName;
                }
            } else {
				LvHelper::log("error", "Module ".$moduleName." doesn't exists; Cannot generate !!!", $comm);
            }
        }
        ...
        try {
            LvHelper::log("line", "Creating migration...", $comm);
            $migrationData = file_get_contents($templateDirectory."/migration.template");
        ...
        LvHelper::log("info", "Migration done: ".$migrationFileName."\n", $comm);
	}
