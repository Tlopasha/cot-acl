...
class LvFormMaker
{
	...
	public static function input($module, $field_name, $default_val = null, $required2 = null, $class = 'form-control', $params = [])
	{
		...
		if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "write")) {
			...
			if($unique && !isset($params['unique'])) {
				...
				$params['adminRoute'] = config('LaravelVueAdmin.adminRoute');
				if(isset($row)) {
					$params['isEdit'] = true;
			...
			switch ($field_type->name) {
				...
				case 'Dropdown':
					...
					if($popup_vals != "") {
						$popup_vals = LvFormMaker::process_values($popup_vals);
					} else {
						$popup_vals = array();
					}
				...
				case 'Taginput':
					...
					$default_val = LvFormMaker::process_values($default_val);
	...
	**/
	// $values = LvFormMaker::process_values($data);
		...
		if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "view")) {
			...
			switch ($field_type->name) {
				...
				case 'Decimal':
					...
					break;
				case 'Dropdown':
					$values = LvFormMaker::process_values($fieldObj['popup_vals']);
	...
	**/
	public static function form($module, $fields = [])
	{
		...
		foreach ($fields as $field) {
			$out .= LvFormMaker::input($module, $field);
		}
		return $out;
	}
	...
	**/
	public static function lv_access($module_id, $access_type = "view", $user_id = 0)
	{
	...
	**/
	public static function lv_field_access($module_id, $field_id, $access_type = "view", $user_id = 0)
	{
