...
class RolesController extends Controller
{
	...
	public function index()
	{
		...
		if(Module::hasAccess($module->id)) {
			return View('LaravelVueAdmin.roles.index', [
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Roles", "create")) {
			...
			return redirect()->route(config('LaravelVueAdmin.adminRoute') . '.roles.index');
	...
	public function show($id)
	{
		if(Module::hasAccess("Roles", "view")) {
			...
			if(isset($role->id)) {
				...
				return view('LaravelVueAdmin.roles.show', [
					'module' => $module,
	...
	public function edit($id)
	{
		if(Module::hasAccess("Roles", "edit")) {
			...
			if(isset($role->id)) {
				...
				return view('LaravelVueAdmin.roles.edit', [
					'module' => $module,
					'view_col' => $this->view_col,
				])->with('role', $role);
			} else {
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) {
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('LaravelVueAdmin.adminRoute') . '/roles/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Roles", "edit")) {
					$output .= '<a href="'.url(config('LaravelVueAdmin.adminRoute') . '/roles/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}

				if(Module::hasAccess("Roles", "delete")) {
					$output .= Form::open(['route' => [config('LaravelVueAdmin.adminRoute') . '.roles.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
	...
	public function save_module_role_permissions(Request $request, $id)
	{
		if(Entrust::hasRole('SUPER_ADMIN')) {
			...
			return redirect(config('LaravelVueAdmin.adminRoute') . '/roles/'.$id.'#tab-access');
		} else {
			return redirect(config('LaravelVueAdmin.adminRoute')."/");
		}
	}
